...
void AuctionHouseBot::addNewAuctions(Player *AHBplayer, AHBConfig *config)
{
    if (!AHBSeller)
    {
        if (debug_Out) sLog->outString("AHSeller: 禁用");
        return;
    }
    ...
    if (debug_Out) sLog->outError( "AHSeller:当前拍卖商 GUID为 %u", AuctioneerGUID);
...
*/
    ...
    if (debug_Out) sLog->outError( "AHSeller: %u 物品", items);

    // only insert a few at a time, so as not to peg the processor
    for (uint32 cnt = 1; cnt <= items; cnt++)
    {
    if (debug_Out) sLog->outError( "AHSeller: %u 数量", cnt);
        uint32 itemID = 0;
        uint32 itemColor = 99;
        ...
        while (itemID == 0 && loopbreaker <= 50)
        {
            ...
            else
            {
                ...
                if (debug_Out) sLog->outError( "AHBuyer: %u 品质不支持", prototype->Quality);
...
void AuctionHouseBot::addNewAuctionBuyerBotBid(Player *AHBplayer, AHBConfig *config, WorldSession *session)
{
    if (!AHBBuyer)
    {
        if (debug_Out) sLog->outError( "AHBuyer: 禁用");
        return;
    }
    ...
    for (uint32 count = 1; count <= config->GetBidsPerInterval(); ++count)
    {
        ...
        if (!pItem)
        {
			if (debug_Out) sLog->outError( "AHBuyer: 物品 %u 不存在，可能已经卖出了", auction->item_guidlow);
            continue;
        }
        ...
        if (debug_Out)
        {
            ...
            sLog->outString("AHBuyer: 拍卖商: %u", auction->auctioneer);
            sLog->outString("AHBuyer: 所有者: %u", auction->owner);
            sLog->outString("AHBuyer: 投标人: %u", auction->bidder);
            sLog->outString("AHBuyer: 开始出价: %u", auction->startbid);
            sLog->outString("AHBuyer: 当前报价: %u", currentprice);
            sLog->outString("AHBuyer: 买入: %u", auction->buyout);
            sLog->outString("AHBuyer: 押金: %u", auction->deposit);
            sLog->outString("AHBuyer: 过期时间: %u", uint32(auction->expire_time));
            sLog->outString("AHBuyer: 买方出价: %f", bidrate);
            sLog->outString("AHBuyer: Bid Max: %Lf", bidMax);
            sLog->outString("AHBuyer: Bid Value: %Lf", bidvalue);
            sLog->outString("AHBuyer: 买入价格: %u", bidprice);
            sLog->outString("AHBuyer: 物品 GUID: %u", auction->item_guidlow);
            sLog->outString("AHBuyer: 物品 模板: %u", auction->item_template);
            sLog->outString("AHBuyer: 物品 信息:");
            sLog->outString("AHBuyer: 物品 ID: %u", prototype->ItemId);
            sLog->outString("AHBuyer: 购买价格: %u", prototype->BuyPrice);
            sLog->outString("AHBuyer: 出售价格: %u", prototype->SellPrice);
...
void AuctionHouseBot::Initialize()
{
    ...
    if ((AHBplayerAccount != 0) || (AHBplayerGUID != 0))
    {
        ...
        if (!result)
       {
           sLog->outError( "AuctionHouseBot: 为您的AHBot设置的帐户/GUID信息不正确 (account: %u guid: %u)", AHBplayerAccount, AHBplayerGUID);
           return;
        }
    }
    ...
    if (AHBSeller)
    {
        ...
        {
            if (debug_Out) sLog->outError( "AuctionHouseBot: \"%s\" 失败", npcQuery);
        }
        ...
        {
            if (debug_Out) sLog->outError( "AuctionHouseBot: \"%s\" 失败", lootQuery);
        }
        ...
        for (ItemTemplateContainer::const_iterator itr = its->begin(); itr != its->end(); ++itr)
        {
            ...
            {
                if (debug_Out_Filters) sLog->outError( "AuctionHouseBot: 物品 %u 禁用 (PTR/Beta/未使用的物品)", itr->second.ItemId);
                continue;
            }
            ...
            {
                if (debug_Out_Filters) sLog->outError( "AuctionHouseBot: 物品 %u 禁用 (永久附魔物品)", itr->second.ItemId);
                continue;
            }
            ...
            {
                if (debug_Out_Filters) sLog->outError( "AuctionHouseBot: 物品 %u 禁用 (施消耗品)", itr->second.ItemId);
                continue;
            }
            ...
            {
                if (debug_Out_Filters) sLog->outError( "AuctionHouseBot: 物品 %u 禁用 (宝石)", itr->second.ItemId);
                continue;
            }
        ...
        sLog->outString("%u 禁用物品", uint32(DisableItemStore.size()));
        sLog->outString("加载 %u 灰色交易物品", uint32(greyTradeGoodsBin.size()));
        sLog->outString("加载 %u 白色交易物品", uint32(whiteTradeGoodsBin.size()));
        sLog->outString("加载 %u 绿色交易物品", uint32(greenTradeGoodsBin.size()));
        sLog->outString("加载 %u 蓝色交易物品", uint32(blueTradeGoodsBin.size()));
        sLog->outString("加载 %u 紫色交易物品", uint32(purpleTradeGoodsBin.size()));
        sLog->outString("加载 %u 橙色交易物品", uint32(orangeTradeGoodsBin.size()));
        sLog->outString("加载 %u 黄色交易物品", uint32(yellowTradeGoodsBin.size()));
        sLog->outString("加载 %u 灰色物品", uint32(greyItemsBin.size()));
        sLog->outString("加载 %u 白色物品", uint32(whiteItemsBin.size()));
        sLog->outString("加载 %u 绿色物品", uint32(greenItemsBin.size()));
        sLog->outString("加载 %u 蓝色物品", uint32(blueItemsBin.size()));
        sLog->outString("加载 %u 紫色物品", uint32(purpleItemsBin.size()));
        sLog->outString("加载 %u 橙色物品", uint32(orangeItemsBin.size()));
        sLog->outString("加载 %u 黄色物品", uint32(yellowItemsBin.size()));
    }
    sLog->outString("拍卖机器人和拍卖买家已经加载完成。");
}
...
{
    if (debug_Out)
		sLog->outError( "开始设置 %s 拍卖行", WorldDatabase.PQuery("SELECT name FROM mod_auctionhousebot WHERE auctionhouse = %u", config->GetAHID())->Fetch()->GetCString());
    if (AHBSeller)
    {
